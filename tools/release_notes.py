...
def main() -> int:
    ...
    lines += [
        ...
        "## 使用",
        "",
        "解压后直接运行 `ghidraRun`（Windows 为 `ghidraRun.bat`）——**开箱即中文**，",
        "不需要任何额外参数。",
        "",
        "想换回英文界面，把 `support/launch.properties` 里的这一行改成 `off` 或删除：",
        "",
        "```properties",
        "VMARGS=-Dghidra.i18n=zh_CN",
        "```",
        "",
        "> 注意：在命令行上写 `ghidraRun -Dghidra.i18n=...` 是**无效的**——`ghidraRun`",
        "> 会把命令行参数拼在类名之后，那属于程序参数，JVM 不会解析。要临时切换请用",
        "> 环境变量 `GHIDRA_JAVA_OPTIONS`。",
        "",
    ]
